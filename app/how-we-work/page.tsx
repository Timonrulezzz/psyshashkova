...
const steps = [
  {
    ...
    text:
      'Вы рассказываете о том, что привело вас сейчас. Я задаю вопросы, помогаю отделить главное от второстепенного и начинаю собирать картину происходящего. Необязательно заранее уметь сформулировать точный запрос. Иногда он становится понятнее уже в разговоре. «Мне плохо, и я пока не очень понимаю почему» тоже может быть достаточной точкой начала.',
  },
  {
    number: '02',
...
const faq = [
  ...
  },
  {
    ...
    a:
      `Лучше предупредить минимум за ${site.practice.cancellation.standardNoticeHours} часов. К экстренным и непредвиденным ситуациям я отношусь спокойно. Если поздние отмены или пропуски начинают повторяться, мы отдельно обсуждаем, как организовать дальнейшие встречи так, чтобы формат оставался рабочим для обеих сторон.`,
  },
  {
  ...
  },
  {
    ...
    a:
      'Нет. Между встречами я могу предлагать наблюдения, упражнения или небольшие эксперименты, если они нужны для нашей задачи. Мы обсуждаем их смысл и подбираем реалистичный объем. Сам факт выполненной домашки не является мерой хорошего клиента.',
  },
  {
  ...
  },
  {
    ...
    a:
      'Это не значит, что психологическая работа прекращается. Можно одновременно работать со мной и наблюдаться у психиатра. Если я вижу, что состояние стоит дополнительно обсудить с психиатром, я скажу об этом прямо. Лекарства и медицинские решения остаются зоной ответственности врача, а мы продолжаем работать со своей частью.',
  },
] as const;
  ...
  <div className="grid gap-4 md:grid-cols-2">
    ...
    <div className="md:pl-0">
      ...
      >
        ...
        <section className="mx-auto max-w-6xl px-6 py-8 md:px-8 md:py-10">
          <Reveal>
            <div className="grid gap-8 md:grid-cols-[0.72fr_1.28fr] md:gap-x-12">
  <div>
    <Eyebrow>Результат</Eyebrow>

    <h2 className="mt-3 max-w-[430px] text-3xl font-normal leading-[1.06] md:text-[38px]">
      Как понять, что работа движется
    </h2>

    <p
      className="mt-5 max-w-[500px] text-[14px] leading-[1.65]"
      style={{ color: C.inkSoft }}
    >
      Улучшение не всегда выглядит как полное исчезновение неприятных
      эмоций. Иногда более важный результат в том, что тревога, стыд,
      раздражение или неопределенность перестают полностью управлять тем,
      что вы делаете.
    </p>
  </div>

  <div className="grid gap-3 self-start md:mt-[58px] md:grid-cols-2">
    {[
      'Сложная ситуация возникает реже или уже не выбивает так сильно.',
      'После сложного момента вы быстрее возвращаетесь в свое обычное состояние.',
      'Между автоматической реакцией и действием появляется больше выбора.',
      'То, что раньше приходилось только понимать, постепенно получается делать иначе в реальной жизни.',
    ].map((item) => (
      <div
        key={item}
        className="flex min-h-[118px] items-center px-5 py-4"
        style={{
          backgroundColor: C.surface,
          borderRadius: radius.md,
        }}
